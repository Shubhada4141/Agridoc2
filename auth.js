...
import { getFirestore, collection, addDoc, getDocs, query, where } from "./firebase.js";

// Authentication System for AgriTech (Firestore version)
class AuthManager {
  constructor() {
  ...
  async initFirestore() {
    // Wait for firebase.js to initialize Firestore
    if (window.db) {
      this.db = window.db;
    } else {
      // Wait for db to be available
      let tries = 0;
      while (!window.db && tries < 10) {
  ...
  async register(userData) {
    ...
    const q = query(collection(this.db, "users"), where("email", "==", email.toLowerCase().trim()));
    ...
    try {
      ...
      return { success: false, message: 'Error creating user: ' + e.message };
    }
  }
